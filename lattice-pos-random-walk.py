...
# x, y - Größe des Gitters
# p - Wahrscheinlichkeit eines leitenden Zustands
# t - Zeitpunkte
# T - Temparatur
# n - Anzahl an Simulationen / Aufteilung des Funktionsparameters
# m - Anzahl an Walkern
...
def plot_gegen_t(n):
  ...
  for t in range(1, n):
    ...
    random_walk = L.random_walk(t, m)
    sigma = L.calc_sigma(random_walk, T, t)

    tvalues.append(t)
    sigmavalues.append(sigma)

  plt.figure(figsize=(8, 6))
  plt.plot(tvalues, sigmavalues, marker='o', linestyle='-')
  plt.xlabel('t')
  plt.ylabel('Sigma (σ)')
...
def plot_gegen_p(n):
  ...
  plt.figure(figsize=(8, 6))
  plt.plot(pvalues, sigmavalues, marker='o', linestyle='-')
  plt.xlabel('t')
  plt.ylabel('Sigma (σ)')
  plt.title('Sigma vs. t')
  plt.grid(True)
